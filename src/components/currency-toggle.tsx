...
export function CurrencyToggle({ className = "" }: { className?: string }) {
  ...
  return (
    <select
      ...
      className={`h-7 rounded-full border border-white/10 bg-black/30 px-2 text-[11px] uppercase tracking-wide text-foreground/80 shadow-sm backdrop-blur-md transition-colors hover:bg-black/40 hover:text-foreground focus:outline-none disabled:opacity-50 ${className}`}
    >
      {currencies.map((c) => (
