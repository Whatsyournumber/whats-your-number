...
function InlineRow({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="grid gap-1 sm:grid-cols-[7.5rem_1fr] sm:items-center sm:gap-3">
      <Label className="min-w-0 text-xs font-medium text-muted-foreground sm:self-center">
        {label}
      </Label>
...
function Select({
  ...
  return (
    <div className="relative">
      <select
        className="h-9 w-full truncate rounded-md border border-input bg-background py-2 pl-3 pr-8 text-sm appearance-none"
        value={value}
...
const PROTECTION_OPTIONS = [0, 50, 60, 70, 75, 80, 85, 90, 95, 100];
...
function InvestmentCard({
  ...
  return (
    <Card title={h.label || t("Activo", "Asset")} onRemove={onRemove} onCollapse={() => setOpen(false)}>
      {/* Campos comunes */}
      <InlineRow label={t("Nombre", "Name")}>
      ...
      {isMarket ? (
        <>
          ...
          <InlineRow label={h.kind === "crypto" ? t("Precio promedio de compra", "Average purchase price") : t("Strike price (precio de compra)", "Strike price")}>
            <DerivedPrice
              value={buyPrice}
              onCommit={(n) => {
