      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        <Link to="/patrimonio" className="block transition-transform hover:-translate-y-0.5">
          <KpiCard
            ...
            valueCompact={fmtCompact(current.netWorth)}
        ...
        <KpiCard label={t("Ingresos", "Income")} value={fmt(current.income)} valueCompact={fmtCompact(current.income)} {...(hasHistory ? { delta: delta(current.income, previous.income) } : {})} icon={Banknote} index={1} />
        <KpiCard label={t("Gastos", "Expenses")} value={fmt(current.expenses)} valueCompact={fmtCompact(current.expenses)} {...(hasHistory ? { delta: delta(current.expenses, previous.expenses) } : {})} inverse icon={TrendingUp} index={2} />
        <KpiCard label={t("Ahorro", "Savings")} value={fmt(current.savings)} valueCompact={fmtCompact(current.savings)} {...(hasHistory ? { delta: delta(current.savings, previous.savings) } : {})} icon={PiggyBank} index={3} />
        ...
        <Link to="/hipoteca" className="block transition-transform hover:-translate-y-0.5">
          <KpiCard
            ...
            valueCompact={fmtCompact(mortgageBalance)}
            hint={mortgageHint}
            inverse
            icon={Home}
            index={5}
            ...
            <div>
              ...
              <p
                className={cn(
                  ...
                  fmt(targetNumber).length > 12 ? "text-xl" : "text-2xl",
                )}
                title={fmt(targetNumber)}
              >
                {fmt(targetNumber).length > 12 ? fmtCompact(targetNumber) : fmt(targetNumber)}
              </p>
