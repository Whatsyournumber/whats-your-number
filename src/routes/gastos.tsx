...
function Gastos() {
  ...
  // Mostrar siempre el gasto real acumulado del periodo (sin proyectar el mes completo)
  const monthlyRun = isLongRange ? monthlyAverage : fixed.total + variableTotal;
  ...
  const runPeriodCompactHint = isLongRange
    ? t(`Promedio ${monthsInRange} meses`, `${monthsInRange}-month average`)
    : runPeriodHint;
  ...
  const runAdvice = async () => {
    ...
    try {
      ...
      const needsUpgrade = /upgrade_required|requires the pro plan|requires the patrimonio plan/i.test(raw);
      if (needsUpgrade) adviceBlocked.current = true;
      setAdviceError(needsUpgrade ? upgrade : raw || generic);
    } finally {
