  ...
  const rows = (list: typeof enriched) => (
    <div className="space-y-2">
      {[...list].sort((a, b) => b.value - a.value).map((h) => {
        const dc = dayChange[h.ticker?.toUpperCase()];
        const hasMarket = dc !== undefined;
        return (
          <div key={h.ticker} className="grid grid-cols-2 items-center gap-3 rounded-xl bg-elevated/60 p-3 md:grid-cols-5">
            ...
            <div>
              ...
              {hasMarket ? (
                <p className={cn("numeric text-sm font-semibold", dc >= 0 ? "text-positive" : "text-negative")}>
                  {dc > 0 ? "+" : ""}
                  {dc.toFixed(2)}%
                </p>
              ) : (
